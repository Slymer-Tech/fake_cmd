...
from slime_core.utils.metaclass import Metaclasses, _ReadonlyAttrMetaclass
...
from fake_cmd.utils import config, version_check
...
from fake_cmd.utils.file import remove_file_with_retry, remove_dir_with_retry
from fake_cmd.utils.exception import CLITerminate, ignore_keyboard_interrupt
from fake_cmd.utils.parallel import LifecycleRun, ExitCallbackFunc
...
class State(ReadonlyAttr):
    ...
    def add_keyboard_interrupt(self, cnt: int = 1) -> None:
        with self.keyboard_interrupt_lock:
            if self.keyboard_interrupt >= self.keyboard_interrupt_max_cnt:
                return
...
def cli_action_version_check(
    ...
    def decorator(func: Callable[..., Union["ClientCommand", Literal[False]]]):
        ...
        def wrapper(
            ...
            if self.version_strict and not version_check(
...
class CLI(
    ...
    def __init__(self, address: str, id_prefix: Union[str, None] = None):
    ...
    def process_cmd(
        ...
        if cli_func is MISSING:
            ...
            args = cmd_parser.parse_args(args=[], strict=False)
            if args is MISSING:
                return False
            ...
            return self.send_session_cmd(
                content=cmd_parser.make_cmd_content(args), type="cmd"
            )
        else:
    ...
    def send_session_cmd(
        self, content: dict, type: str = "cmd"
    ) -> Union["ClientCommand", Literal[False]]:
        """
        ...
        msg = CommandMessage(
            session_id=session_info.session_id, type=type, content=content
        )
    ...
    def ls_server_version(self, cmd_splits: CMDSplit) -> Literal[False]:
        logger.info(f"Client version: {config.version}")
        return False
    ...
    def interactive_cmd(
        self, cmd_splits: CMDSplit
    ) -> Union["ClientCommand", Literal[False]]:
        """
        Open command in an interactive mode.
        """
        cmd_parser = cmd_parser_registry.get("inter")()
        args = cmd_parser.parse_cmd(cmd_splits=cmd_splits, strict=False)
        if not args:
            return False
    ...
    def pexpect_cmd(
        ...
        return self.send_session_cmd(
            content=cmd_parser.make_cmd_content(args), type="pexpect_cmd"
        )

    @cli_registry.register_multi(["ls-back", "ls-session", "ls-cmd"])
    @cli_action_version_check(min_version=(0, 0, 1))
    ...
    def server_shutdown(self, cmd_splits: CMDSplit) -> Literal[False]:
        try:
            ...
            if res != confirm_key:
                logger.info("Server shutdown canceled.")
                return False
...
class Client(
    ...
    def __init__(
        self, cli: CLI, address: str, id_prefix: Union[str, None] = None
    ) -> None:
        """
    ...
    def before_running(self) -> bool:
        ...
        if not os.path.exists(server_check_fp):
            logger.warning(f"Server address not found: {address}.")
        ...
        for _ in polling():
            try:
                ...
                if to_be_destroyed and not msg:
                    ...
                    continue
                action = dispatch_action(self.client_registry, msg.type, f"Client")
    ...
    def cmd_quit(self, msg: Message) -> None:
        ...
        if current_cmd and cmd_id == current_cmd.cmd_id:
    ...
    def connect(self) -> bool:
        ...
        if not res:
            return False
        if not wait_symbol(self.session_info.conn_client_fp):
            logger.warning(
    ...
    def disconnect(self, initiator: bool):
        ...
        if initiator:
            with state.terminate_lock:
                ...
                if not wait_symbol(disconn_confirm_to_client_fp) or not wait_symbol(
                    disconn_client_fp
                ):
                    logger.warning(
    ...
    def check_connection(self) -> bool:
        ...
        if check_symbol(disconn_client_fp):
            ...
            return False
        elif not self.heartbeat.beat() or self.state.cli_exit.is_set():
...
class ClientCommand(
    ...
    readonly_attr__ = ("cli", "msg", "cmd_input")
    ...
    def running(self):
        ...
        def _check_cmd_state() -> bool:
            ...
            if state.cmd_finished.is_set():
                ...
                return False
            if (
                state.cmd_terminate_remote.is_set()
                or state.terminate.is_set()
                or to_be_terminated
            ):
        ...
        def _send_kill_cmd_msg(type: str):
            ...
            """
            if self.version_strict and not version_check(
                self.server_version, min_version=(0, 0, 2)
            ):
                return
            send_msg_to_session(
                type="kill_cmd", content={"cmd_id": msg.cmd_id, "type": type}
            )
        ...
        def _process_keyboard_interrupt():
            ...
            if kill_disabled:
                ...
                send_msg_to_session(
                    type="background_cmd", content={"cmd_id": msg.cmd_id}
                )
                logger.info(
    ...
    def redirect_output(
        ...
        # ``not content``.
        if (read_all and (not content)) or ((not read_all) and content is False):
            return False
...
class InteractiveInput(
    ...
    def running(self) -> None:
        logger.info(f"You are in the interactive mode.")
        send_msg_to_session = self.send_msg_to_session
        state = self.state
        while True:
            try:
                ...
                send_msg_to_session(
                    type="cmd_input",
                    content={"cmd_id": self.cmd_id, "input_str": input_str},
                )
            except KeyboardInterrupt:
                with ignore_keyboard_interrupt():
                    if not self.cmd_running:
