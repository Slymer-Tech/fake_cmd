...
from fake_cmd.utils.parallel import CommandPool, LifecycleRun, ExitCallbackFunc
...
class SessionState(ReadonlyAttr):
    ...
    readonly_attr__ = ("unable_to_communicate",)
...
class CommandState(ReadonlyAttr):
    ...
    def pending_terminate(self) -> bool:
        return (
            self.terminate_local.is_set()
            or self.terminate_remote.is_set()
            or self.terminate_disconnect.is_set()
            or self.force_kill.is_set()
            or (self.keyboard_interrupt.is_set() and not self.kill_disabled.is_set())
        )
...
class Server(
    LifecycleRun, ReadonlyAttr, metaclass=Metaclasses(ABCMeta, _ReadonlyAttrMetaclass)
):
    readonly_attr__ = ("server_info", "session_dict", "max_cmds")

    # Dispatch different message types.
    action_registry = Registry[ActionFunc]("server_action")

    def __init__(self, address: str, max_cmds: Union[int, None] = None) -> None:
    ...
    def destroy_session(self, msg: Message):
        ...
        if session_id not in self.session_dict:
            logger.warning(f"Session id {session_id} not in the session dict.")
            return
        destroy_fp = SessionInfo(
            self.server_info.address, session_id
        ).destroy_session_fp
    ...
    def server_shutdown(self, msg: Message):
        logger.info(f'Server shutdown received from: "{msg.session_id}".')
        raise ServerShutdown

    def _cleanup_dead_sessions(self):
        """Remove finished session processes from the dict."""
        dead = [sid for sid, proc in self.session_dict.items() if not proc.is_alive()]
        for sid in dead:
    ...
    def check_action_result(self, res: Union[None, Tuple[str, ...]]):
        if res is None:
            ...
            logger.warning(f"Missing args: {res}")
...
class Session(
    ...
    def __init__(
        ...
        self.session_listener = MessageChannel(
            self.session_info.session_queue_namespace
        )
    ...
    def running(self):
        ...
        self.send_msg_to_client(
            type="server_version", content={"version": list(config.version)}
        )
        ...
        for _ in polling():
            ...
            action = dispatch_action(
                action_registry, msg.type, f"Server Session {session_id}"
            )
            if action is not MISSING:
                res = action(self, msg)
    ...
    def run_new_cmd(self, msg: Message):
        ...
        cmd = PopenShellCommand(
            session=self, msg=msg, exit_callbacks=[terminate_command_func]
        )
    ...
    @param_check(
        required=["cmd", "encoding", "reader", "kill_disabled", "interactive", "echo"]
    )
    ...
    def input_to_cmd(self, msg: Message):
        ...
        with self.running_cmd_lock:
            ...
            if not cmd_state.executor_writable.is_set():
                self.info_client(
                    f"Input failed: Current cmd {str(running_cmd)} does not accept input."
                )
                return
            if not cmd_state.executor_started.is_set():
                self.info_client(
                    f"Input failed: Command {str(running_cmd)} has not started yet."
                )
                return
            if cmd_state.finished.is_set() or cmd_state.exit.is_set():
                self.info_client(
                    f"Input failed: Command {str(running_cmd)} is not running."
                )
                return
    ...
    def notify_cmd_inconsistency(
        self, running_cmd: "SessionCommand", incoming_cmd_id: str
    ):
        """
    ...
    def safely_terminate_cmd(
        ...
        with self.running_cmd_lock:
            ...
            if self.running_cmd is not cmd:
                logger.warning(f"Running cmd inconsistency occurred.")
            if not cmd:
                return
    ...
    def set_running_cmd(self, running_cmd: Union["SessionCommand", None]):
        ...
        """
        with self.running_cmd_lock:
            if self.running_cmd and running_cmd:
    ...
    def connect(self) -> bool:
        ...
        if not wait_symbol(self.session_info.conn_session_fp):
            logger.warning(
    ...
    def disconnect(self, initiator: bool):
        ...
        if initiator:
            ...
            if not wait_symbol(disconn_confirm_to_session_fp) or not wait_symbol(
                disconn_session_fp
            ):
                logger.warning(
            ...
            if not wait_symbol(disconn_confirm_to_session_fp):
                ...
                # Set the client is unable to communicate.
                self.session_state.unable_to_communicate.set()
        logger.info(f"Successfully disconnect session: {self.session_id}")
    ...
    def check_connection(self) -> bool:
        ...
        with self.running_cmd_lock:
            if check_symbol(disconn_session_fp):
                ...
                os.path.exists(self.session_info.destroy_session_fp)
                or not self.heartbeat.beat()
            ):
    ...
    def clear_cache(self):
        ...
        wait_symbol(
            self.session_info.disconn_confirm_to_client_fp, wait_for_remove=True
        )
        # Clear anyway.
        self.session_info.clear_session()
    ...
    def check_action_result(self, res: Union[None, Tuple[str, ...]], msg: Message):
        if res is None:
            ...
            if msg.type in ("cmd", "inner_cmd", "pexpect_cmd"):
                ...
                self.send_msg_to_client(
                    type="cmd_quit", content={"cmd_id": msg.msg_id, "type": "remote"}
                )
    ...
    def send_msg_to_client(self, type: str, content: Union[dict, None] = None) -> bool:
        ...
        self.client_writer.write(
            Message(session_id=self.session_id, type=type, content=content)
        )
...
def _session_process_entry(
    address: str, session_id: str, max_cmds: Union[int, None] = None
):
    """
    ...
    session = Session(session_info=session_info, max_cmds=max_cmds)
    session.run()
...
class _SessionProxy:
    ...
    def send_msg_to_client(self, type: str, content: Union[dict, None] = None):
        self.client_writer.write(
            Message(session_id=self.session_info.session_id, type=type, content=content)
        )
...
def _shell_command_process_entry(
    ...
    cmd = cmd_cls(session=proxy, msg=msg, cmd_state=cmd_state)
    cmd.run()
...
class SessionCommand(
    ...
    * ``InnerCommand`` → child ``Thread`` (no fork involved)
    ...
    readonly_attr__ = ("session", "msg", "executor", "cmd_state", "output_writer")
    ...
    def after_running(self, __exc_type=None, __exc_value=None, __traceback=None):
        ...
        if (
            cmd_state.terminate_remote.is_set()
            or cmd_state.force_kill.is_set()
            or cmd_state.keyboard_interrupt.is_set()
        ):
            create_symbol(session_info.command_terminate_confirm_fp(self.msg))
        elif cmd_state.terminate_local.is_set():
            self.send_msg_to_client(
                type="cmd_quit", content={"cmd_id": self.cmd_id, "type": "remote"}
            )
        elif cmd_state.finished.is_set():
            self.send_msg_to_client(
                type="cmd_quit", content={"cmd_id": self.cmd_id, "type": "finish"}
            )
        ...
        if __exc_type or __exc_value or __traceback:
            self.info_client(
...
class ShellCommand(SessionCommand):
    ...
    def running(self) -> None:
        ...
        def _process_kill_signals():
            ...
            if (
                state.terminate_local.is_set()
                or state.terminate_disconnect.is_set()
                or state.force_kill.is_set()
            ) and not kill_sent:
                # Directly kill the process.
                executor.kill()
                kill_sent = True
            elif state.terminate_remote.is_set() and not terminate_sent:
            ...
            elif state.keyboard_interrupt.is_set() and not interrupt_sent:
        ...
        with executor:
            ...
            for _ in polling(config.cmd_polling_interval):
                ...
                _process_input()
                # Write outputs.
                self.output_writer.write(executor.read(config.cmd_pipe_read_timeout))
...
class PopenShellCommand(ShellCommand):
    ...
    def __init__(
        ...
        reader: PopenReader = popen_reader_registry.get(
            content["reader"], PipePopenReader
        )()
        writer: PopenWriter = popen_writer_registry.get(
            content["writer"], PopenWriter
        )()
...
class PexpectShellCommand(ShellCommand):
    ...
    def __init__(
        ...
        content: dict = msg.content
        reader: PexpectReader = pexpect_reader_registry.get(
            content["reader"], DefaultPexpectReader
        )()
        encoding: str = content["encoding"] or config.cmd_pipe_encoding
        executor = pexpect_executor_registry.get("default", PexpectExecutor)(
            pexpect_params=FuncParams(content["cmd"]),
            reader=reader,
            encoding=encoding,
            echo=content["echo"],
        )
...
class InnerCommand(SessionCommand):
    ...
    def list_background(self):
        ...
        if len(background_cmds) == 0:
            output_writer.print("(No background cmds available)")
            return
    ...
    def list_session(self):
        ...
        try:
            session_dirs = [
                d
                for d in os.listdir(address)
                if d.endswith(SESSION_ID_SUFFIX)
                and os.path.isdir(os.path.join(address, d))
            ]
        except OSError:
            session_dirs = []
    ...
    def update_command_states__(self):
        with self.lock:
            self.set_list__(
                list(
                    filter(
                        lambda cmd: not cmd.cmd_state.exit.is_set(), self.get_list__()
                    )
                )
            )
