...
from slime_core.utils.typing import Missing, MISSING, Union, Callable, Any, Tuple, Dict
...
class ServerInfo(ReadonlyAttr):
    ...
    readonly_attr__ = ("address", "listen_fname", "listen_namespace")
    ...
    def init_server(self):
        ...
        for session_fname in filter(
            lambda fname: fname.endswith(SESSION_ID_SUFFIX), os.listdir(self.address)
        ):
...
class SessionInfo(ServerInfo, ReadonlyAttr):
    ...
    readonly_attr__ = ("address", "session_id")
    ...
    def __init__(self, address: str, session_id: str) -> None:
        ...
        self.disconn_confirm_to_session_fp = self.concat_session_path(
            "session.disconn.confirm"
        )
        self.disconn_client_fp = self.concat_session_path("client.disconn")
        self.disconn_confirm_to_client_fp = self.concat_session_path(
            "client.disconn.confirm"
        )
        # Heartbeat.
    ...
    def init_server(self):
        logger.warning(f"``init_server`` should not be called in the ``SessionInfo``.")

    def clear_server(self):
        logger.warning(f"``clear_server`` should not be called in the ``SessionInfo``.")
...
def dispatch_action(
    registry: Registry[ActionFunc], type: str, caller: str
...
def param_check(required: Tuple[str, ...]):
    """
    ...
    def decorator(func: ActionFunc) -> ActionFunc:
        ...
        def wrapper(self, msg: Message) -> Union[None, Tuple[str, ...]]:
            ...
            missing_params = tuple(filter(lambda param: param not in content, required))
