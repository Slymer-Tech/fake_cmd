...
from slime_core.utils.typing import List, Union, TYPE_CHECKING, Callable, Iterable, Type
from . import config
...
class ExitCallbacks:
    ...
    def __init__(
        self, callbacks: Union[Iterable[ExitCallbackFunc], None] = None
    ) -> None:
...
class CommandPool(ReadonlyAttr):
    ...
    readonly_attr__ = ("queue_lock", "execute_lock", "pool_close", "polling_thread")

    def __init__(self, max_threads: Union[int, None] = None) -> None:
    ...
    def schedule(self):
        ...
        with self.queue_lock, self.execute_lock:
            ...
            while self.queue and self.max_threads > len(self.execute):
    ...
    def update_and_get_execute(self) -> List["SessionCommand"]:
        with self.execute_lock:
            self.execute = list(filter(lambda exec: exec.is_alive(), self.execute))
            return self.execute
